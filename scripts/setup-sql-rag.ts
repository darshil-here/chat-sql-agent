/**
 * Setup script: runs all initialization steps in order.
 * Usage: npx tsx scripts/setup-sql-rag.ts
 *
 * Steps:
 * 1. Apply database migrations
 * 2. Seed database with faker data (~8,335 rows)
 * 3. Seed FTS5 schema metadata
 * 4. Rebuild FTS5 index (clean, without columns/relationships)
 */
...
async function applyMigrations() {
  console.log("1️⃣  Applying migrations...");
...
async function seedDatabase() {
  console.log("2️⃣  Seeding database with faker data...");
  const { execSync } = await import("node:child_process");
  execSync("npx tsx db/db.seed.ts", { stdio: "inherit", cwd: process.cwd() });
...
async function seedMetadata() {
  console.log("3️⃣  Seeding FTS5 schema metadata...");
  ...
  console.log(`   ✅ Metadata seeded (${tableMetadata.length} tables)\n`);
}

async function rebuildFTS5() {
  console.log("4️⃣  Rebuilding FTS5 index...");

  await db.run(sql`DROP TABLE IF EXISTS schema_metadata_fts`);
  await db.run(sql`
  ...
  console.log("   ✅ FTS5 rebuilt (indexed: table_name, description, example_queries)\n");
}

async function main() {
  console.log("🚀 QueryMind SQL RAG Setup\n");
